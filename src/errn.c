	...
	mpfr_prec_t *prec, int var)
{
	...
	*/
	...
	mpfr_printf("%d\t%d\t%d\t%.20Rg\t%.20Rg\t%.20Rg\t%.20Rg\t%.10Rg\n",
		phh[var], var, gold->n, err1, err, norm2_err, norm2_rel, cond);
...
void test_rnd(gmp_randstate_t state, int n)
{
	...
	mpfr_prec_t prec_set[SIZE_PREC_SET] = {
		4, 8, 12, 16, 20, 24, 32, 48, 52, 64};
